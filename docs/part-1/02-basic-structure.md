...
Another version of TIMES is available, in which the TIMES variables (capacities and flows) are defined at some year in the midst of each period (called milestone year), and are assumed to evolve linearly between the successive milestone years. This option emulates that of the EFOM model and is discussed in section {numref}`%s <the-linear-variant-of-times>`.
...
[^4]: However, there are TIMES variants -- discussed in chapters {numref}`%s <stochastic-programming-extension>` to {numref}`%s <general-equilibrium-extensions>`, that depart significantly from these assumptions
