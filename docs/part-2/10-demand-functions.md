...
For the own-price elasticities, we have the following relations (see Part I, Chapter 4), where $U_{i}$ is the term in the objective function associated with the utility change due to the demand variation of demand *i*:
  ...
  \begingroup\f@size{4}$U_{i} = \sum_{t}^{}\left( \frac{p_{i}^{0}(t)}{(1 + 1/E_{i})} \cdot \left\lbrack DM_{i}^{0}(t) \right\rbrack^{- 1/E_{i}} \bullet DM_{i}(t)^{1 + 1/E_{i}} \right)$\endgroup     \(3\)
  -----------------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
The share parameters $α_{i}$ can be derived from the expenditure shares, as shown in Eq. (7) below. In the objective function, the utility change can then be calculated by the expression shown in Eq. (9) below.
